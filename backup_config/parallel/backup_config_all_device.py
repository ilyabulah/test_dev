...
from backup_config_zte import backup_config_zte
from backup_config_cisco_router import backup_config_cisco_router
...
def move_config_to_nas_for_zte(hostname, directory):

    shutil.copy(f"/var/tftp/{hostname}.dat", f"/mnt/backup_config/{directory}/{today}/{hostname}.dat")
...
if __name__ == "__main__":
    ...
    with open("ulan-ude.yaml") as f:
        dev_uu = yaml.safe_load(f)

    with open("baikal.yaml") as f:
        dev_baikal = yaml.safe_load(f)
    
    region = {'muh': dev_muh, 'sag': dev_sag, 'gus': dev_gus, 'uu': dev_uu, 'baikal': dev_baikal}
    #region = {'muh': dev_muh}
    name = {'muh': 'п.Мухоршибирь', 'sag': 'п.Саган-Нур', 'gus': 'г.Гусиноозерск, п.Селендума', 'uu': 'г.Улан-Удэ', 'baikal': 'п.Селенгинск'}
    ...
    print('\n')
    print(fg(46) + f'Запуск скрипта выполняющего резервное копирование конфигураций с оборудования {today}' + fg.rs)
    print('\n')
    ...
    for directory, devices in region.items():
        ...
        for dev in devices:
            ...
            if dev['host'] not in unreachable_ip:
                ...
                elif dev['device_type'] == 'zte_zxros':
                    list_dict.append(dev)
                    list_files.append('zte_zxros')
                    with open('zte_zxros.yaml', 'a') as f:
                        yaml.dump(list_dict, f)

                elif dev['device_type'] == 'cisco_ios':
                    list_dict.append(dev)
                    list_files.append('cisco_ios')
                    with open('cisco_ios.yaml', 'a') as f:
                        yaml.dump(list_dict, f)
        ...
        if 'zte_zxros' in list_files:
            with open("zte_zxros.yaml") as f:
                dev_switch_zte = yaml.safe_load(f)
            os.remove("zte_zxros.yaml")

            with ThreadPoolExecutor(max_workers=21) as executor:
                results = executor.map(backup_config_zte, dev_switch_zte)
                for output in results:
                    if type(output) is dict:
                        move_config_to_nas_for_zte(output['hostname'], directory)
                    else:
                        ip_con_error.append(output)

        if 'cisco_ios' in list_files:
            with open("cisco_ios.yaml") as f:
                dev_cisco_router = yaml.safe_load(f)
            os.remove("cisco_ios.yaml")

            with ThreadPoolExecutor(max_workers=21) as executor:
                results = executor.map(backup_config_cisco_router, dev_cisco_router)
                for output in results:
                    if type(output) is dict:
                         write_to_txt_file(output)
                    else:
    ...
    if not_backup_ip:
        ...
        os.chdir(f"/mnt/backup_config/!log")
